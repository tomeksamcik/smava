 ...
 * @author Tomek Samcik
 *
 *         Accounts controller class, exposes bank accounts CRUD operations via
 *         RESTful API
 */
@Controller
@Api(value = "/account")
...
public class AccountEndpoint {
	 ...
	 * 
	 * @param session
	 *            a session to retreive accounts for
	 * @return a list of accounts
	 * @throws AccountServiceException
	 *             a generic service layer exception
	 */
	@ApiOperation(httpMethod = "GET", value = "Finds Accounts in session", notes = "Session is populated with initial data on the first call", response = Account.class, responseContainer = "List")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Success"),
			@ApiResponse(code = 500, message = "Unexpected server error"), })
	@RequestMapping(method = RequestMethod.GET)
	public @ResponseBody List<Account> get(HttpSession session)
			throws AccountServiceException {
		init(session);
	 ...
	 *            entity to create
	 * @param session
	 *            session to create the entity for
	 * @return created entity
	 * @throws AccountServiceException
	 *             a generic service layer exception
	 */
	@ApiOperation(httpMethod = "POST", value = "Creates a new Account in session", notes = "No duplicated IBAN numers are allowed for a session", response = Account.class)
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Success"),
	 ...
	 * @param session
	 *            a session to update entity for
	 * @return an updated entity
	 * @throws AccountServiceException
	 *             a generic service layer exception
	 */
	@ApiOperation(httpMethod = "PUT", value = "Updates an Account in session", notes = "No duplicated IBAN numers are allowed for a session", response = Account.class)
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Success"),
	 ...
	 * @param session
	 *            a session to delete from
	 * @throws AccountServiceException
	 *             a generic service layer exception
	 */
	@ApiOperation(httpMethod = "DELETE", value = "Deletes an Account in session")
