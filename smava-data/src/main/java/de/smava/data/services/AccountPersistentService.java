...
public class AccountPersistentService implements AccountService {
	 ...
	 * 
	 * @see
	 * de.smava.data.AccountService#getAccounts(javax.servlet.http.HttpSession)
	 */
	public List<Account> getAccounts(HttpSession session)
			throws AccountServiceException {
		try {
			Optional<HttpSession> sessionOrNull = Optional.ofNullable(session);
			return accountRepository.findAllBySessionId(sessionOrNull.map(
					s -> s.getId()).orElse(null));
		} catch (Exception e) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.smava.data.services.AccountService#getAccount(javax.servlet.http.
	 * HttpSession, java.lang.Long)
	 */
	@Override
	public Account getAccount(HttpSession session, Long id)
		...
		try {
			...
			Account account = accountRepository.findByIdAndSessionId(id,
					sessionOrNull.map(s -> s.getId()).orElse(null));
			if (account != null) {
				return account;
			} else {
				throw new AccountServiceEntityNotFoundException(
						"Entity with the given id (" + id + ") not found");
			}
	 ...
	 * 
	 * @see
	 * de.smava.data.AccountService#addAccount(javax.servlet.http.HttpSession,
	 * de.smava.data.Account)
	 */
	public Account addAccount(HttpSession session, Account account)
			throws AccountServiceException {
		try {
			Optional<HttpSession> sessionOrNull = Optional.ofNullable(session);
			account.setSessionId(sessionOrNull.map(s -> s.getId()).orElse(null));
			return accountRepository.save(account);
		} catch (DataIntegrityViolationException e) {
			throw new AccountServiceNonUniqueException(
					"Non unique iban value (" + account.getIban() + ")");
		} catch (Exception e) {
			throw new AccountServiceException(e.getMessage());
		}
	}
	 ...
	 * 
	 * @see
	 * de.smava.data.AccountService#deleteAccount(javax.servlet.http.HttpSession
	 * , java.lang.Long)
	 */
	public void deleteAccount(HttpSession session, Long id)
		...
		try {
			if (id != null) {
				Optional<HttpSession> sessionOrNull = Optional
						.ofNullable(session);
				Account toBeDeleted = accountRepository.findByIdAndSessionId(
						id, sessionOrNull.map(s -> s.getId()).orElse(null));
				if (toBeDeleted != null) {
				...
				throw new AccountServiceException(
						"Id has to be specified for delete operation");
			}
		} catch (AccountServiceEntityNotFoundException e) {
			throw new AccountServiceEntityNotFoundException(e.getMessage());
		} catch (Exception e) {
	 ...
	 * 
	 * @see
	 * de.smava.data.AccountService#updateAccount(javax.servlet.http.HttpSession
	 * , java.lang.Long, de.smava.data.Account)
	 */
	public Account updateAccount(HttpSession session, Long id, Account account)
		...
		try {
			if (id != null || account != null) {
				Optional<HttpSession> sessionOrNull = Optional
						.ofNullable(session);
				Account toBeUpdated = accountRepository.findByIdAndSessionId(
						id, sessionOrNull.map(s -> s.getId()).orElse(null));
				if (toBeUpdated != null) {
				...
				throw new AccountServiceException(
						"Id and account have to be specified for update operation");
			}
		} catch (DataIntegrityViolationException e) {
			throw new AccountServiceNonUniqueException(
					"Non unique iban value (" + account.getIban() + ")");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.smava.data.services.AccountService#deleteAll(javax.servlet.http.
	 * HttpSession)
	 */
