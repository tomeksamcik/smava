...
public class AccountSessionService implements AccountService {
	 ...
	 * 
	 * @param session
	 *            session to get accounts from
	 * @return a list of accounts
	 * @throws IOException
	 *             deserialization error
	 */
	private List<Account> getFromSession(HttpSession session)
			throws IOException {
	 ...
	 * 
	 * @param session
	 *            sessopm tp push account to
	 * @param accounts
	 *            a list of accounts
	 * @throws IOException
	 *             serialization exception
	 */
	private void putInSession(HttpSession session, List<Account> accounts)
			throws IOException {
		if (session.getAttribute("accounts") == null) {
			session.setAttribute("accounts", "[]");
		}
	...
	public Account getAccount(HttpSession session, Long id)
		...
		try {
			...
			if (!filtered.isEmpty()) {
				if (filtered.size() == 1) {
					...
					throw new AccountServiceException(
							"More then one entity found with the given id ("
									+ id + ")");
				}
			} else {
			...
			throw new AccountServiceException(
					"Error serializing/deserializing accounts: "
							+ e.getMessage());
		} catch (Exception e) {
		...
		try {
			...
			if (!filtered.isEmpty()) {
				if (filtered.size() == 1) {
					...
					List<Account> filteredIban = accounts.stream()
							.filter(a -> a.getIban().equals(account.getIban()))
							.collect(Collectors.toList());
